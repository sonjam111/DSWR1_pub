...
The same but now we use the pipe operator `%>%` which has the effect of making the output to its left an input to its right.
...
Without the pipe operator, in order to obtain the same output:
